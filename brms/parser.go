...
func (p *Parser) Parse() (*ParsedBRMS, error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "[") && strings.HasSuffix(line, "]") {
			...
			if len(parts) > 2 {
				return nil, &BRMSError{Line: lineNumber, Message: "invalid format with multiple ':' in block section"}
			}
			if parts[1] == "" {
			...
			if len(parts) > 2 {
				return nil, &BRMSError{Line: lineNumber, Message: "invalid format with multiple ':' in entity mapping"}
			}
			if parts[1] == "" {
				// Exclusion case.
				parsed.IgnoredItems = append(parsed.IgnoredItems, Exclusion{Source: parts[0]})
